...
namespace AssetsTools.NET.Atomic
{
    ...
    {
        public RefTypeManager GetRefTypeManager(AtomicAssetsFileInstance inst)
        {
        ...
        public AssetTypeTemplateField GetTemplateBaseField(
            AtomicAssetsFileInstance inst, AssetFileInfo info,
        ...
        public AssetTypeTemplateField GetTemplateBaseField(
            AtomicAssetsFileInstance inst, AssetsFileReader reader, long absByteStart,
            int typeId, ushort scriptIndex, AssetReadFlags readFlags)
        {
            ...
            if (typeId == (int)AssetClassID.MonoBehaviour && MonoTempGenerator != null && !skipMonoBehaviourFields && reader != null)
            {
                ...
                if (!msPtr.IsNull())
                {
                    AtomicAssetsFileInstance monoScriptFile;
                    if (msPtr.FileId == 0)
                        monoScriptFile = inst;
                    else
        ...
        private bool GetMonoScriptInfo(
            AtomicAssetsFileInstance inst, long absFilePos, int typeId, ushort scriptIndex,
        ...
        public AssetTypeTemplateField CreateTemplateBaseField(AtomicAssetsFileInstance inst, int id, ushort scriptIndex = 0xffff)
        {
            AssetsFile file = inst.file;
        ...
        public AssetTypeValueField CreateValueBaseField(AtomicAssetsFileInstance inst, int id, ushort scriptIndex = 0xffff)
        {
        ...
        public AssetTypeValueField GetBaseField(AtomicAssetsFileInstance inst, AssetFileInfo info, AssetReadFlags readFlags = AssetReadFlags.None)
        {
        ...
        public AssetTypeValueField GetBaseField(AtomicAssetsFileInstance inst, long pathId, AssetReadFlags readFlags = AssetReadFlags.None)
        {
