...
namespace AssetsTools.NET.Atomic
{
    ...
    {
        public void LoadDependencies(AtomicAssetsFileInstance ofFile)
        {
        ...
        public void LoadBundleDependencies(AtomicAssetsFileInstance ofFile, AtomicBundleFileInstance ofBundle, string path)
        {
            for (int i = 0; i < ofFile.file.Metadata.Externals.Count; i++)
            {
                string depPath = ofFile.file.Metadata.Externals[i].PathName;
